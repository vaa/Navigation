    ...
    methods: {
        ...
        search(pageCode, pageSize) {
            this.$http.post(api.user.findByPage(pageSize, pageCode), this.searchEntity).then(result => {
                this.user = result.body.data.rows;
        ...
        save() {
            if (this.editor.name == null || this.editor.name == '' || this.editor.url == null || this.editor.url == '') {
                ...
                this.$http.post(api.user.save, JSON.stringify(this.editor)).then(result => {
                    this.reloadList();
                    if (result.body.code == 200) {
                        this.editor.user = {};
                        this._notify(result.body.msg, 'success')
        ...
        handleEdit(id) {
            ...
            this.editor = {}; //清空表单
            //查询当前id对应的数据
            this.$http.get(api.user.findById(id)).then(result => {
        ...
        edit() {
            this.editDialog = false;
            //查询当前id对应的数据
            this.$http.put(api.user.update, JSON.stringify(this.editor)).then(result => {
                this.reloadList();
                if (result.body.code == 200) {
                    this._notify(result.body.msg, 'success')
                } else {
