var app = new Vue({
    ...
    data: {
        ...
        localUpload: api.admin.localUpload,
        avatarDialog: false,
        avatarList: [],
        defaultActive: '6',
    ...
    methods: {
        ...
        //获取当前用户信息
        getUserInfo() {
            this.$http.get(api.admin.info).then(result => {
                this.admin = result.body.data;
                this.pass.id = result.body.data.id;
            });
        },

        save() {
            this.$http.post(api.admin.update, JSON.stringify(this.admin)).then(result => {
        ...
        handleEditAvatar() {
            this.$http.get(api.admin.avatar).then(response => {
        ...
        //修改头像
        changeAvatar(url) {
            this.admin.avatar = url;
            var data = {
                id: this.admin.id,
                avatar: this.admin.avatar
            };
            this.$http.post(api.admin.update, JSON.stringify(data)).then(response => {
                this.avatarDialog = false;
        ...
        changePassword() {
            if (this.pass.password.length < 6) {
                ...
                this.$http.post(api.admin.update, JSON.stringify(this.pass)).then(result => {
                    if (result.body.code == 200) {
                        this._notify(result.body.msg, 'success');
                        //执行/logout请求
                        window.alert("更改了密码，请重新登录！");
